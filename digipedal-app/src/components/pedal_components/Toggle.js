...
function Toggle({x, y, width, height, toggled, setToggled}) {
  return (
    <svg x={x} y={y} width={width} height={height} viewBox={`0 0 ${width} ${height}`} fill="none">
        ...
        <circle cx={(toggled ? width - height/2 : height/2)} cy={height/2} r={height/2} fill={toggled ? "#2E772C" : "#95290E"} onClick={setToggled}/>
    </svg>
  );
} 
