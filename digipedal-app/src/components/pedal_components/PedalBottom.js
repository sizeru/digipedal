...
function PedalBottom({width, height, startHeight, toggled, setToggled}) {
    ...
    let svg_output = (
        <svg x={0} y={startHeight} width={width} height={height} viewBox={`0 0 ${width} ${height}`} fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <Toggle x={width/2 - width / 6} y={height/2 - height/3} width={width / 3} height={height * 2/3} toggled={toggled} setToggled={toggle}/>
            {/* info button */}
            <InfoButton cx={width * 5/6} cy={height/2} r = {height/3} showInfo={info}/>
